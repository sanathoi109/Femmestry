...
# -----------------------------
# Model constants (easy to manage)
# -----------------------------
VISION_MODEL = "qwen/qwen3.6-27b"          # For receipt scanning (image + text)
TEXT_MODEL = "llama-3.3-70b-versatile"     # For financial coaching (text-only)
...
# -----------------------------
# Budget: Scan Receipt (Vision Model)
# -----------------------------
...
"""
        ...
        completion = groq_client.chat.completions.create(
            model=VISION_MODEL,   # ✅ Vision model for receipts
            messages=[
                {
                    "role": "user",
                    "content": [
...
def rag_explain():
    try:
        ...
        print("RAG query from frontend:", repr(user_query))
        ...
        system_prompt = (
            ...
            "You speak in simple, clear language, use one practical example from daily Indian life, "
            "and you never show your internal reasoning or thinking steps. "
            "You only return the final explanation and guidance."
        )
...
"""
        ...
        response = groq_client.chat.completions.create(
            model=TEXT_MODEL,   # ✅ Text model for coaching
            messages=[
