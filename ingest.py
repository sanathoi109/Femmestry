...
def run_ingestion():
    ...
    # 2. Read the curriculum JSON file
    ...
    # 4. Process and format items for ChromaDB
    ids = []
    documents = []
    metadatas = []
    ...
    for item in curriculum_data:
        ids.append(item["id"])
        # Format the text chunk that will be embedded and retrieved
        doc_text = f"Topic: {item['topic']}\nContent: {item['content']}"
        documents.append(doc_text)
        
        # Metadata helps us filter results by user levels
        metadatas.append({
            "topic": item["topic"],
            "level": int(item.get("level", 1))
        })

    # 5. Insert / Upsert into vector store
    collection.upsert(
        ids=ids,
        documents=documents,
        metadatas=metadatas
